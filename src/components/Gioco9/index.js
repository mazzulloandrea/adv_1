...
function Gioco9({ data, onend }) {
  ...
  const durataEffective =
    durata || (livello === 1 ? 5 : livello === 2 ? 10 : 15);
  let svgSet;
  if (dataset === "combat") {
    svgSet = combatSet;
  ...
  useEffect(() => {
    ...
    if (
      ["memory", "domanda", "numbers"].includes(type) &&
      errors.length === HowManyError
    ) {
      verify();
    }
  ...
  const drawDomanda = () => {
    if (type === "domanda") {
      return html` <div class=${style.domanda}>${domanda}</div> `;
    }
    if (type === "numbers") {
      return domanda.map((d) => html` <div class=${style.domanda}>${d}</div> `);
    }
  };
    ...
    return cubes.map(
      (cube) =>
        html`
          <div
            ...
            onclick=${(evt) => {
              ...
              if (type === "memory") {
                ...
                if (
                  randomCombinazione.slice(0, candidate.length).join("") ===
                  candidate.join("")
                ) {
                ...
                randomCombinazione.slice(0, candidate.length) ===
                candidate.join("")
              ) {
                setCubesClicked(candidate);
                const styled = evt.currentTarget.style;
          ...
          >
            ${type === "memory"
              ? html`<${svgSet[cube]} class=${style.svgIcon} />`
              : html`<span>${cube}</span>`}
          </div>
        `
    );
